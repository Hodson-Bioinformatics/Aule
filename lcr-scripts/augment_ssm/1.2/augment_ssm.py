...
verbose = True #turn off print statements
...
##### DEVELOP INPUT ##### 
# augmented_maf = "~/rds/rds-hodson-rds-elshhQhj4ec/runs/DIRECT/20240509_lowVAF_test/DIR1001_test_augmented.maf"
# tumour_bam_file = "~/rds/rds-hodson-rds-elshhQhj4ec/runs/DIRECT/20240402_FULLRUN_V3/results/aule_preproc-1.0/99-outputs/capture--hg38/DIR1001_Plasma_B_v1.bam"
# normal_bam_file = "~/rds/rds-hodson-rds-elshhQhj4ec/runs/DIRECT/20240402_FULLRUN_V3/results/aule_preproc-1.0/99-outputs/capture--hg38/DIR1001_Germline_B_v1.bam"
# index_maf_file = "~/rds/rds-hodson-rds-elshhQhj4ec/runs/DIRECT/20240402_FULLRUN_V3/results/slms_3-1.0_vcf2maf-1.3/99-outputs/deblacklisted/maf/capture--hg38/DIR1001_Biopsy_B_v1--DIR1001_Germline_B_v1--matched.matched.slms-3.final.maf"
# add_maf_files = ["~/rds/rds-hodson-rds-elshhQhj4ec/runs/DIRECT/20240402_FULLRUN_V3/results/slms_3-1.0_vcf2maf-1.3/99-outputs/deblacklisted/maf/capture--hg38/DIR1001_Biopsy_B_v1--DIR1001_Germline_B_v1--matched.matched.slms-3.final.maf",
#                 "~/rds/rds-hodson-rds-elshhQhj4ec/runs/DIRECT/20240402_FULLRUN_V3/results/slms_3-1.0_vcf2maf-1.3/99-outputs/deblacklisted/maf/capture--hg38/DIR1001_Plasma_B_v1--DIR1001_Germline_B_v1--matched.matched.slms-3.final.maf"]
# min_map_qual = 40
# min_base_qual = 20
# padding = 100
# tumour_sample_id = "DIR1001_Plasma_B_v1"
# nproc = 1
# genome_build = "hg38"
...
# 
# # Params
# min_map_qual = snakemake.params["min_map_qual"]     # Min MAPQ
# min_base_qual = snakemake.params["min_base_qual"]   # Min base quality
# padding = snakemake.params["padding"]               # Padding
# tumour_sample_id = snakemake.wildcards["tumour_id"] # Sample ID compatibile with the tumour bam file
##### 
...
# Generate a pileup and get the ref and alt read count for each variant (row) in all rows
...
def get_read_support(chrom, start, end, ref_base, alt_base, bamfile, mut_class, min_base_qual=10, min_mapping_qual=1):
    ...
    return({'depth':str(total_depth),'ref_count':str(ref_count),'alt_count':str(alt_count)})

# Define the unique set of variants to check (union of the index maf and the other mafs)
def subset_and_run(full_index_maf, this_chromosome, other_maf_files):
    
    # Merge the index maf and the other mafs, keep unique rows 
    maf_list = [pd.read_csv(file, sep="\t", comment='#', dtype={'Chromosome':'string', 't_ref_count':'string', 't_alt_count':'string', 't_depth':'string'}, low_memory=False) for file in other_maf_files]
    maf_list.append(full_index_maf)
    combined_df = pd.concat(maf_list, ignore_index=True)
    combined_df = pd.concat(maf_list, ignore_index=True).drop_duplicates()
    print(combined_df.shape[0]) # Total number of unique 
