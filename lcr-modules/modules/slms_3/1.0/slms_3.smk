...
rule _slms_3_annotate_strelka_gnomad:
    ...
    shell:
        ...
        bcftools view -i 'INFO/AF < 0.0001  && FMT/DP[1] >= 10' -Oz -o {output.vcf} 2> {log.stderr}
        && 
        tabix -p vcf {output.vcf} 2>> {log.stderr}
        """) # removed "&& INFO/SomaticEVS >= 10" from bcftools view command due to loss of indels
...
rule _slms_3_mutect2_depth_filt: 
    ...
    shell: 
        ...
        bcftools view  -s "NORMAL,TUMOR" -i 'FMT/DP[@{input.table}] >= 10 && FMT/AD[@{input.table}:1] >= 4' 
        -Oz -o {output.vcf} 2> {log.stderr} && 
        tabix -p vcf {output.vcf} 2>> {log.stderr}
        """)
        # removed FMT/AF filter, original cmd: bcftools view  -s "NORMAL,TUMOR" -i 'FMT/DP[@{input.table}] >= 10 && FMT/AD[@{input.table}:1] >= 4 && FMT/AF[@{input.table}:0] >= 0.1' 
