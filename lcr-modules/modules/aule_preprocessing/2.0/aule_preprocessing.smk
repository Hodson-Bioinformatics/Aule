...
#
# 2.0 redesigns UMI handling to read UMIs inline from R1/R2 (fgbio
# ExtractUmisFromBam) instead of from a separate UMI FASTQ (fgbio
# AnnotateBamWithUmis, used by 1.0). See README.md for the full comparison and
...
CFG = op.setup_module(
    ...
    version = "2.0",
    subdirectories = ["inputs", "fastq_to_unmapped_bam", "extract_umis_from_bam", "mark_illumina_adapters", "bwa_input", "bwa_mem", "merge_bam_alignment", "fix_tags", "mark_duplicates", "base_recalibration", "apply_bsqr", "outputs"],
)
...
# Only R1/R2 -- unlike 1.0, no separate UMI FASTQ is needed since UMIs are read
# inline from the read sequence (see _aule_preprocessing_extract_umis_from_bam).
...
# Extract UMIs from inline read structure (5M2S+T 5M2S+T) directly into the RX
# tag. Replaces 1.0's _aule_preprocessing_annotate_bam_with_umis (fgbio
# AnnotateBamWithUmis against a separate UMI FASTQ). Note: fgbio
# ExtractUmisFromBam has no option to write a UMI base-quality tag (unlike
# AnnotateBamWithUmis's -q RQ) -- see README.md for what this means for RQ.
rule _aule_preprocessing_extract_umis_from_bam:
    priority: -7
    input:
        ubam = str(rules._aule_preprocessing_fastq_to_unmapped_bam.output.ubam)
    output:
        ubam = temp(CFG["dirs"]["extract_umis_from_bam"] + "{seq_type}/{sample_id}.unmapped_umi.bam")
    log:
        stdout = CFG["logs"]["extract_umis_from_bam"] + "{seq_type}/{sample_id}.extract_umis.stdout.log",
        stderr = CFG["logs"]["extract_umis_from_bam"] + "{seq_type}/{sample_id}.extract_umis.stderr.log"
    params:
        options = CFG["options"]["extract_umis_from_bam"],
        scratch = CFG["scratch_directory"],
        java = CFG["options"]["java_fgbio"]
    conda:
        CFG["conda_envs"]["fgbio"]
    threads:
        CFG["threads"]["extract_umis_from_bam"]
    resources:
        **CFG["resources"]["extract_umis_from_bam"]
    shell:
        op.as_one_line("""
...
# Find adapters (run after UMI extraction; optional but recommended)
rule _aule_preprocessing_mark_illumina_adapters:
    priority: -6
    input:
        ubam = str(rules._aule_preprocessing_extract_umis_from_bam.output.ubam)
    output:
    ...
    params:
        options = CFG["options"]["mark_illumina_adapters"],
        scratch = CFG["scratch_directory"]
    conda:
        CFG["conda_envs"]["picard"]
    threads:
...
# BWA input (convert marked UMI uBAM to interleaved FASTQ)
...
# Merge ubam and bam (use UMI-tagged unaligned BAM)
...
# Mark Duplicates (UMI-aware via RX tag)
