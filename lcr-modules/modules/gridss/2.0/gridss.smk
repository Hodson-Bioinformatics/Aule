...
def get_split_vcf(wildcards): 
    ...
    tbi = expand(
        str(rules._gridss_split_vcf.output.tbi), 
        patient_id = TUMOUR['tumour_patient_id'], 
        allow_missing = True
    )
...
rule _gridss_output_somatic_vcf:
    input:
        ...
        ### bedpe = str(rules._gridss_gripss_to_bedpe.output.bedpe) ### COMMENTED BY IM
    output:
        ...
        filtered_tbi = CFG["dirs"]["outputs"] + "vcf/{seq_type}--{genome_build}/{tumour_id}--{normal_id}--{pair_status}.gridss_somatic_filtered.vcf.gz.tbi" ### COMMENTED BY IM, 
        ### bedpe = CFG["dirs"]["outputs"] + "bedpe/{seq_type}--{genome_build}/{tumour_id}--{normal_id}--{pair_status}.gridss_somatic_filtered.bedpe" ### COMMENTED BY IM
    run:
        ...
        ### op.relative_symlink(input.bedpe, output.bedpe, in_module=True) ### COMMENTED BY IM
    ...
    input:
        expand(
            [
                ...
                str(rules._gridss_output_somatic_vcf.output.somatic_tbi)###, ### COMMENTED BY IM
                ###str(rules._gridss_output_somatic_vcf.output.bedpe) ### COMMENTED BY IM
            ],
