...
unless( $inhibit_vep ) {
    ...
    # Add AlphaMissense plugin (GRCh38/hg38 only) when available
    if ( $ncbi_build =~ /38/ && -e "../../../reference/AlphaMissense/AlphaMissense_hg38.tsv.gz" ) {
        $vep_cmd .= " --dir_plugins ../../../reference/downloads/vep_plugins";
        $vep_cmd .= " --plugin AlphaMissense,file=../../../reference/AlphaMissense/AlphaMissense_hg38.tsv.gz";
    }
