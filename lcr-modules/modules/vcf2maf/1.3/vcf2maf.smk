...
rule _vcf2maf_run:
    ...
    shell:
        ...
        vepPATH=$(dirname $(which vep))/../share/ensembl-vep*;
...
f = open(CFG["conda_envs"]["gamblr"], 'rb')
md5hash.update(f.read())
f.close()
h = md5hash.hexdigest()
#GAMBLR = glob.glob(conda_prefix + "/" + h[:8] + "*")
GAMBLR = conda_prefix + "/" + h + "_"
GAMBLR_dir = GAMBLR + "/lib/R/library/GAMBLR"
...
rule _vcf2maf_install_GAMBLR:
    params:
        ...
        gamblrdir = CFG["lcr-modules"] + "../GAMBLR"
    output:
        installed = directory(GAMBLR_dir),
        config = "gamblr.yaml"
    log:
        stdout = CFG["logs"]["vcf2maf"] + "install_GAMBLR.stdout.log",
        stderr = CFG["logs"]["vcf2maf"] + "install_GAMBLR.stderr.log",
    conda:
