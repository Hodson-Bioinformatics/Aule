...
CFG = op.setup_module(
    name = "ichorcna_pon_offtarget",
    version = "1.0",
    subdirectories = ["inputs", "offtarget_bam", "bamCoverage", "bigwigToWig", "convert_wig", "compile_wigs", "ichorcna_pon", "outputs"],
)

localrules:
    _ichorcna_pon_offtarget_input_bam,
    _ichorcna_pon_offtarget_output_pon,
    _ichorcna_pon_offtarget_all
...
# Install ichorCNA
rule _ichorcna_pon_offtarget_install_ichorcna:
    params:
...
rule _ichorcna_pon_offtarget_input_bam:
    input:
...
rule _ichorcna_pon_offtarget_exclude_panel:
    input:
        bam = str(rules._ichorcna_pon_offtarget_input_bam.output.bam),
        bed = CFG["inputs"]["panel_bed"]
    output:
        bam = temp(CFG["dirs"]["offtarget_bam"] + "{seq_type}--{genome_build}/{sample_id}_offtarget.bam"),
        bai = temp(CFG["dirs"]["offtarget_bam"] + "{seq_type}--{genome_build}/{sample_id}_offtarget.bam.bai")
    conda:
    ...
    log:
        CFG["logs"]["offtarget_bam"] + "{seq_type}--{genome_build}/{sample_id}_offtarget.log"
    shell:
        op.as_one_line("""
...
rule _ichorcna_pon_offtarget_bamCoverage:
    input:
        bam = str(rules._ichorcna_pon_offtarget_exclude_panel.output.bam),
        bai = str(rules._ichorcna_pon_offtarget_exclude_panel.output.bai)
    output:
...
rule _ichorcna_pon_offtarget_bigwigToWig:
    input:
        bw = str(rules._ichorcna_pon_offtarget_bamCoverage.output.bw)
    output:
...
rule _ichorcna_pon_offtarget_convert_wig:
    input:
        wig = str(rules._ichorcna_pon_offtarget_bigwigToWig.output.wig)
    output:
...
def get_chrom_wigs_pon_offtarget(wildcards):
    CFG = config["lcr-modules"]["ichorcna_pon_offtarget"]
...
rule _ichorcna_pon_offtarget_compile_wigs:
    input:
        wigs = get_chrom_wigs_pon_offtarget
    output:
...
# Generate a list of all wig files for the PON
rule _ichorcna_pon_offtarget_wigpaths:
    input:
        bw = expand(
            [
                str(rules._ichorcna_pon_offtarget_compile_wigs.output.wig)
            ],
            zip,
...
def get_gcwig_pon_offtarget(wildcards):
    genome_ucsc = op.switch_on_wildcard("genome_build", config["lcr-modules"]["ichorcna_pon_offtarget"]["options"]["ichorcna_pon"]["genome_ucsc"])(wildcards)
    return ichorcna_dir + "/inst/extdata/gc_" + genome_ucsc + "_" + wildcards.binSize + "kb.wig"

def get_mapwig_pon_offtarget(wildcards):
    genome_ucsc = op.switch_on_wildcard("genome_build", config["lcr-modules"]["ichorcna_pon_offtarget"]["options"]["ichorcna_pon"]["genome_ucsc"])(wildcards)
    return ichorcna_dir + "/inst/extdata/map_" + genome_ucsc + "_" + wildcards.binSize + "kb.wig"

def get_centromere_pon_offtarget(wildcards):
    centromere = op.switch_on_wildcard("genome_build", config["lcr-modules"]["ichorcna_pon_offtarget"]["options"]["ichorcna_pon"]["centromere"])(wildcards)
    return ichorcna_dir + "/inst/extdata/" + centromere


# Generate PON
rule _ichorcna_pon_offtarget_create_pon:
    input:
        wigs = str(rules._ichorcna_pon_offtarget_wigpaths.output.txt),
        ichorCNA = str(rules._ichorcna_pon_offtarget_install_ichorcna.output.ichorcna)
    output:
    ...
    params:
        ...
        gcwig = get_gcwig_pon_offtarget,
        mapwig = get_mapwig_pon_offtarget,
        centromere = get_centromere_pon_offtarget,
...
rule _ichorcna_pon_offtarget_output_pon:
    input:
        pon_rds = str(rules._ichorcna_pon_offtarget_create_pon.output.pon_rds),
        pon_txt = str(rules._ichorcna_pon_offtarget_create_pon.output.pon_txt)
    output:
...
rule _ichorcna_pon_offtarget_all:
    input:
        expand(
            [
                str(rules._ichorcna_pon_offtarget_output_pon.output.pon_rds),
                str(rules._ichorcna_pon_offtarget_output_pon.output.pon_txt)
            ],
            zip,
            seq_type = CFG["samples"]["seq_type"],
            genome_build = CFG["samples"]["genome_build"],
