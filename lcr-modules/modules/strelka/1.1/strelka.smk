...
rule _strelka_filter_combine:
    ...
    shell:
        ...
        bcftools filter -i 'INFO/SomaticEVS>0.2' -m x -Ov |
        bcftools sort --max-mem {params.mem_mb}M -Oz -o {output.vcf}
        > {log.stdout} 2> {log.stderr} && 
        tabix -p vcf {output.vcf} >> {log.stdout} 2>> {log.stderr} 
        """)
# Added a customised filter for EVS, setting all variants passing it to PASS. It by-passes an internal Strelka filter, which is difficult to change  
...
def _strelka_get_output(wildcards):
    ...
    if wildcards.pair_status == "no_normal":
       vcf = expand(CFG["dirs"]["filtered"] + "{{seq_type}}--{{genome_build}}/{{tumour_id}}--{{normal_id}}--{{pair_status}}/combined.passed.vcf.gz",
                    var_type = "variants"
        )
    else:
