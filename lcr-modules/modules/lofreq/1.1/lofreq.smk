...
rule _lofreq_preprocess_normal:
    ...
    shell:
        op.as_one_line("""
        lofreq somatic {params.opts} --threads {threads} -t {input.normal_bam} -n {input.normal_bam}
    ...
    #     """) ### modified, removing --normal_only parameter
...
rule _lofreq_run_tumour_unmatched:
    input:
        # preprocessing_complete = str(rules._lofreq_link_to_preprocessed.output.preprocessing_complete), # commented out
        tumour_bam = CFG["dirs"]["inputs"] + "bam/{seq_type}--{genome_build}/{tumour_id}.bam",
        normal_bam = CFG["dirs"]["inputs"] + "bam/{seq_type}--{genome_build}/{normal_id}.bam",
        # vcf_relaxed = str(rules._lofreq_link_to_preprocessed.output.vcf_relaxed), # ommented out
        fasta = reference_files("genomes/{genome_build}/genome_fasta/genome.fa"),
        dbsnp = reference_files("genomes/{genome_build}/variation/dbsnp.common_all-151.vcf.gz"), #in our experience, this filter doesn't remove as many SNPs as one would expect
        bed = _lofreq_get_capspace
    output:
        vcf_relaxed = CFG["dirs"]["lofreq_somatic"] + "{seq_type}--{genome_build}/{tumour_id}--{normal_id}--{pair_status}/normal_relaxed.vcf.gz", # added
