...
SHM_genes <- drivers_list$Gene[drivers_list$SHM_gene == "SHM"]
...
for(ii in seq_along(mafs)){
    temp <- readr::read_delim(mafs[ii]) %>%
        dplyr::select(Hugo_Symbol, Chromosome, Start_Position,
                      ...
                      HGVSc, HGVSp, Variant_Classification, Variant_Type, Consequence,
...
df <- vcf_df %>%
    ...
    dplyr::mutate( # deletions
        ...
        HG38_uniqueID = ifelse(vcf.start != POS, paste0(CHROM, "_", vcf.start, "_", vcf.REF, "_", vcf.ALT), HG38_uniqueID), # amended for joining
...
df %>%
    ...
    dplyr::select(-c(HG38_uniqueID, Variant_Type,
