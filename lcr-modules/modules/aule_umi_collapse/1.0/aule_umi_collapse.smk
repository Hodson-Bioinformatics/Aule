...
CFG = op.setup_module(
    ...
    version = "1.0",
    subdirectories = ["inputs", "keep_proper_pairs", "sortbam", 
    "set_mate_info", "group_by_umi", "collapse_consensus_reads",
    "bwa", "merge_bam_alignment", "sortbam", "outputs"],
)
...
rule _aule_umi_collapse_sortbam_queryname:
    ...
    output:
        bam = temp(CFG["dirs"]["sortbam"] + "{seq_type}--{genome_build}/{sample_id}_properpairs_byqueryname.bam")
    log:
        stdout = CFG["logs"]["sortbam"] + "{seq_type}--{genome_build}/{sample_id}.sortbam_queryname.log",
        stderr = CFG["logs"]["sortbam"] + "{seq_type}--{genome_build}/{sample_id}.sortbam_queryname.stderr.log"
    params:
...
rule _aule_umi_collapse_ubam_sort_queryname:
    input:
        ubam = str(rules._aule_umi_collapse_consensus_reads.output.ubam)
    output:
        ubam = temp(CFG["dirs"]["sortbam"] + "{seq_type}--{genome_build}/{sample_id}.sorted.ubam")
    log:
...
# Sort by query name (picard)
rule _aule_umi_collapse_sort_queryname:
    input:
...
rule _aule_umi_collapse_output_bam:
    input:
        bam = str(rules._aule_umi_collapse_sort_queryname.output.bam),
    output:
