...
checkpoint _mutect2_input_chrs:
    ...
    run:
        ...
        ### ADDED
        # If the file is empty or only contains comments, pd.read_csv will raise an EmptyDataError
        # In this case, return a dataframe with a single entry
        def read_csv_with_comments(filepath, **kwargs):
            try:
                return pd.read_csv(filepath, **kwargs)
            except pd.errors.EmptyDataError:
                return pd.DataFrame(['chr1', 'chr2', 'chr3', 'chr4', 'chr5', 'chr6', 'chr7', 'chr8', 'chr9', 'chr10', 'chr11', 'chr12', 'chr13', 'chr14', 'chr15', 'chr16', 'chr17', 'chr18', 'chr19', 'chr20', 'chr21', 'chr22', 'chrX', 'chrY'])
        ...
        candidate_chrs = read_csv_with_comments(input.candidate_positions, comment='#', sep='\t', header=None) ### MODIFIED BY IM
...
rule _mutect2_run_matched_unmatched:
    input:
        ...
        #pon = reference_files("genomes/{genome_build}/gatk/mutect2_pon.{genome_build}.vcf.gz"), # Changed - PON is now a parameter
        candidate_positions = CFG["inputs"]["candidate_positions"] if CFG["inputs"]["candidate_positions"] else str(rules._mutect2_dummy_positions.output),
        capture_arg = _mutect_get_capspace
    output:
        vcf = CFG["dirs"]["mutect2"] + "{seq_type}--{genome_build}/{tumour_id}--{normal_id}--{pair_status}/chromosomes/{chrom}.output.vcf.gz",
        tbi = CFG["dirs"]["mutect2"] + "{seq_type}--{genome_build}/{tumour_id}--{normal_id}--{pair_status}/chromosomes/{chrom}.output.vcf.gz.tbi",
        stat = CFG["dirs"]["mutect2"] + "{seq_type}--{genome_build}/{tumour_id}--{normal_id}--{pair_status}/chromosomes/{chrom}.output.vcf.gz.stats", 
    ...
    shell:
        ...
        -pon {params.pon} --f1r2-tar-gz {output.f1r2}
